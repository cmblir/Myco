...
- **Display** — *화살표*, *시맨틱 링크*, **번들 스트랜드**(두 토픽 클러스터 사이의 모든 링크를 가중치 3단계의 발광 아크 하나로 묶어, 클러스터는 분리된 채 토픽 간 구조가 읽히게 유지), **노드 색상** 모드(*커뮤니티* 색조 / *화이트* 단색 / *자동* — 노드 수 임계 이하는 화이트), *라벨 페이드 임계*, *노드 크기*, *링크 두께*, **Glow** 슬라이더, **Ambient motion** 토글(자동 회전 + 펄스 + 호흡 애니메이션을 한 스위치로), 그리고 **▶ 타임랩스 재생** 버튼.
- **Layout** — **레이아웃 엔진** 전환(**Galaxy** — 살아있는 3D 포스 시뮬레이션; **Atlas** — 커뮤니티별 반투명 영역 채움이 있는 정적 2D ForceAtlas2 맵, Gephi 스타일)과 세 가지 포스 프리셋(**Galaxy** / **Loose web** / **Dense**), 원본 d3-force 슬라이더(*중심력 / 반발력 / 링크 장력 / 링크 거리 / 클러스터 포스*)는 **Advanced** 아코디언 아래에 있습니다.
