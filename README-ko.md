...
**Ingest** — 파일을 드롭하거나 텍스트를 붙여넣으면 먼저 판정(잡음·중복 본문·
너무 작음·너무 큼)을 거치고, 통과한 것만 `raw/`에 저장되어 활성 모델이
인용·로그·WHY 리포트와 함께 `wiki/`에 통합합니다. 잡음과 NOOP 계획은 파일을
하나도 만들지 않습니다 — 토스트 하나와 `.myco/ingest-noop.jsonl` 한 줄만
남깁니다. Ingest 페이지는 판정 단계, 제외 항목과 이유, 세션 큐의 백필 패널을
보여줍니다. 입력은 멀티모달:
PDF, Office 문서, 스프레드시트, 이미지(비전 프로바이더), 오디오/비디오(내장 음성
인식), YouTube URL. 내장 오프라인 임베딩 인덱스(e5-small-ko, 인프로세스
llama.cpp)가 시맨틱 검색과 페이지별 관련 노트를 제공합니다.

**수확 큐** — 개요 화면은 위키 페이지가 될 만한 세션부터 보여줍니다:
중복·보일러플레이트를 걸러낸 아카이브 대화를 위키 클러스터에 대조하고, 각각의
예상 인용 수를 붙입니다. 원하는 것을 체크하고 계획 게이트("아직 아무것도 만들지
않았습니다")를 확인한 뒤 수확하면 `_inbox/`로 복사·인제스트되고 인용 수가
움직입니다.

**Ask & 에이전트 모드** — 연결된 어떤 모델로든 위키와 대화합니다. 검색은
기본적으로 위키만 대상으로 하고, 범위 스위치로 세션 또는 전체로 넓힐 수 있으며
설정에서 아카이브된 세션까지 인덱스에 포함할 수 있습니다. 모든 답변은 출처
사다리를 보여줍니다 — 계층 칩(노트 / 지도 / 다이제스트 / 세션 / 원본), 신뢰
가중치, 인용된 줄, 계층 사전확률이 각 결과를 얼마나 움직였는지 — 고급 패널에서
사전확률을 조정할 수 있습니다. 관련도 바닥선을 넘는 근거가 없으면 답을 만들지
않고 그렇게 말하며, 아깝게 미달한 후보를 바닥선 게이지에 보여주고 세션까지 넓혀
검색하거나 질문을 수확 대상으로 표시하는 선택지를 줍니다. 에이전트
...
자라난 균사 매트 등 대체 레이아웃 엔진 포함. ~1만 노드까지 60fps. 무대 위의 네
질문 — 어디가 비었나 · 무엇이 뭉쳐 있나 · 최근 무엇이 자랐나 · 이 노트의 이웃 —
은 레이아웃이 아니라 *인코딩*(색 · 크기 · 흐림 · 링)만 바꾸므로 답끼리 비교가
됩니다. 왼쪽 고정 열에는 빈 곳이 나열되고 행마다 세 가지 출구(열기 · 링크 제안 ·
수확 대상으로)가 있으며, 질문 칩 아래 한 줄이 지금 화면의 몇 %가 아직 첫 실행
샘플 vault인지 밝힙니다.
...
**음성 캡처** — `⌥M`(전역 단축키, 어느 앱에서든 동작)으로 녹음하고 Enter로
저장하면, 말하는 동안 받아쓴 자막이 실시간으로 뜨고, 받아쓴 노트가 `_inbox/`에
떨어져 평소의 ingest 파이프라인을 탑니다. 녹음은 노치에서 진행되고, 노치 표면을
꺼 두었다면 Spotlight가 대신 열려 그곳에서 녹음합니다. 음성 인식은 내장되어
있습니다 — whisper.cpp가 앱에 함께 실리고, 모델(~190MB)은 첫 녹음 때
한 번만 내려받으며 진행률이 캡처 화면에 표시됩니다. 설치할 것이 없고 음성은
기기 밖으로 나가지 않습니다. PATH에 이미 `whisper`가 있으면 그쪽을 씁니다.

**리더** — CodeMirror 소스 / 라이브 미리보기 / 분할, `[[위키링크]]` 자동완성,
`/` 블록, ⌘F 찾기/바꾸기, 이미지 붙여넣기(`assets/`), 클릭하면 이동하는 개요 패널, 백링크·관련 노트 패널. 파일 트리는 ⌘/⇧-클릭으로 여러 항목을 선택하고 이동…, 즐겨찾기, 최근 수정 그룹을 제공하며 ⌘[ / ⌘]로 앞뒤로 이동합니다. 실시간 편집(기본)은 제목·글머리 기호·할 일 체크박스·
링크를 제자리에서 렌더링하고 커서가 있는 줄에서만 원본 마크다운을 보여주며,
⌘-클릭으로 위키링크를 엽니다. 프론트매터는 에디터 위의 **속성** 폼으로 편집합니다 —
`type` / `status` / `confidence`는 드롭다운, `source_count`는 숫자, `tags`는
칩, 나머지는 키/값 행이며, 모든 변경은 YAML 블록만 고치는 한 번의 실행 취소
가능한 편집입니다. `raw/` PDF는 인앱 pdf.js 뷰어로 열립니다: 텍스트 선택 →
...
먼저 알려주고, 앱을 종료하기 전까지 되돌릴 수 있습니다. 저장된 뷰는 번들에
들어가지 않습니다 — vault 안에 파일로 살기 때문에(`.myco/views/*.json`,
대시보드 옆) vault와 함께 이동합니다.

**노치 드롭 서피스** *(macOS, 옵트인)* — 맥북 노치 아래 조용히 사는 드롭
타깃. 화면 맨 위로 파일을 끌고 가면 펼쳐지고, 놓으면 `_inbox/`에 앉아
인제스트가 주워 가며, 패널은 스스로 접힙니다. 마우스를 올리면 녹음·노트 버튼이
나와, 앱을 거치지 않고 그 자리에서 음성 메모를 시작할 수 있습니다. 기다리는
결정이 있으면 몇 건인지 알려 주고(불 켜진 점과 숫자, 없으면 아무것도 그리지
않습니다), 마우스를 올리면 최대 세 건을 한 줄씩 내놓습니다 — 토픽 맵 승인,
제안된 연결 걸기, 위키에 들어갈 세션 수확, 또는 무시까지 창을 열지 않고
처리합니다. 포커스를 뺏지 않고 메뉴바
