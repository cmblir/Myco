...
**음성 캡처** — Spotlight나 노치에서 `⌥M`으로 녹음하고 Enter로 저장하면, 말하는
동안 받아쓴 자막이 실시간으로 뜨고, 받아쓴 노트가 `_inbox/`에 떨어져 평소의
ingest 파이프라인을 탑니다. 음성 인식은
내장되어 있습니다 — whisper.cpp가 앱에 함께 실리고, 모델(~190MB)은 첫 녹음 때
