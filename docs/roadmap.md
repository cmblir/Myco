...
| **P1 — phase 1 DONE** | **Wikification pipeline v2** — chunk → schema-constrained extract → embed+retrieve candidate pages → LLM ADD/UPDATE/MERGE/NOOP → cite (select-then-generate) | The single-prompt "Claude, ingest this" approach does not scale to long transcripts or dedup across hundreds of sessions; this is the quality core. **Phase 1 (retrieval grounding) shipped**: `pipeline::rank_candidates` + `wikify_candidates` chunk the source (`embeddings::chunk_page`), embed with the index's own model, retrieve top-k existing pages (`VectorStore::search`) and inject them into the ingest prompt so the agent updates rather than duplicates — the dedup-across-sessions core, entirely on the existing local-embedding infra (no new dep). **Phase 2 (pending)**: promote extract + ADD/UPDATE/MERGE/NOOP into explicit Rust stages with per-decision telemetry. | **L** | `src-tauri/src/pipeline.rs`, `commands::wikify_candidates`, `ingestStore.ts`; embeddings via the bundled local model (`local_llm.rs`) |
