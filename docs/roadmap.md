# myco Roadmap — Conversation & Session Wikification

## Where myco is today

myco is a Tauri 2 (Rust) + React desktop app over a plain-markdown Obsidian vault, which hosts its MCP server in-process (`app/src-tauri/src/mcp_native.rs`, rmcp over streamable HTTP at `localhost:22360/mcp`). *(2026-09-06: the standalone Python/FastMCP server this document was written against — `mcp-server/myco_mcp.py` — is retired; every tool it had lives in `mcp_native.rs`. Treat every `myco_mcp.py` reference below as historical.)* The vault schema is enforced by `CLAUDE.md`: `raw/` is immutable sources, `wiki/` holds LLM-maintained pages with YAML frontmatter, `[^src-*]` inline citations, and `[[wikilinks]]`, and every ingest writes an `ingest-reports/` WHY report. The `Ingest` page (`app/src/pages/PageIngest.tsx`) takes one dropped/pasted document, writes `raw/<slug>.md`, then runs the selected provider (for `anthropic-cli`, the streaming `claude` CLI in `src-tauri/src/claude.rs`) with a fixed `INGEST_PROMPT` so the model maintains the wiki per `CLAUDE.md`. Success is verified by snapshotting `wiki/` mtimes (`file_mtimes`) before/after. The MCP server exposes the durable write primitives — `add_raw_source` (refuses overwrite = immutability), `create_page`, `update_page`, `git_commit`, `_safe_wiki_path`/`is_protected_raw` guards, multi-project registry. Provider-wise, only the CLI agents are tool-capable (can write the vault); HTTP providers (`providers.rs`) are read-only chat.

**Ingest is no longer only manual.** An inbox exists and two things drain it: `app/src/lib/autoIngest.ts` (in-process, while the app is open, on an interval set by `settings.auto_ingest_interval_min`) and `automation/autoingest.py` (headless daemon). Both watch `<vault>/_inbox/`. They diverge: the daemon accepts `.md/.txt/.csv/.json/.yaml/.html` plus PDF/spreadsheets (shelling out to `myco --extract-text`) and archives a consumed source to `_inbox/.archived/`; the in-app pass sees **only `.md`** (`list_files` is markdown-only) and **deletes** the source. `schedules.rs` + `PageSchedules.tsx` add scheduled runs.

**What is actually missing for the owner's goal:** no conversation/session parsers (ChatGPT/Claude/Claude Code/Codex), no dedup/idempotency ledger, no batch/bulk importer, and no chunk→extract→merge pipeline. A vendor export is therefore one enormous "source" rather than N conversations — and `.jsonl` (every Claude Code and Codex session) is handled by **neither** drain path, so those files are currently inert wherever you put them. Re-dropping the same file duplicates rather than deduping.
...
> Several P0 rows say the parsers live in `mcp-server/importers/` **plus a
> "mirror in Rust"** — i.e. two implementations of every vendor format. That is
> the drift risk this document flags at the end, written into the plan as if it
> were settled. It is not: see the open question below the table. Do not start a
> P0 row until the parser home is decided.
...
| **P0** | **Dedup ledger** — SHA-256 of normalized transcript → `.myco/ledger.json`, check-before-write so re-imports are no-ops | Owner re-drops the same export repeatedly; without idempotency `raw/` fills with duplicates and wiki double-counts | **S** | Rust command `ingest_ledger_check`/`record` + MCP tool `ledger_status` |
| **P0** | **Standalone CLI batch importer** — `myco-import <dir|file> --vault <path> --provider …` walks an export, parses, writes `raw/conversations/<src>/<id>.md`, ingests, commits | The "files in, wiki out" automation the owner asked for; works headless, scriptable, no GUI needed for the initial 1000-conversation backfill | **L** | CLI script (`mcp-server/myco_import.py`, reuses MCP write tools in-process) |
| **P0** | **MCP import tools** — `import_conversation(raw_text, source, meta)`, `import_session(jsonl_path)`, `wikify_pending()` | Lets any MCP client (Claude Desktop/Code) drive imports + wikification using the existing `add_raw_source`/`create_page` guards; the agent-native path | **M** | MCP tools in `myco_mcp.py` |
| ~~**P1**~~ **DONE (partly)** | **Drop-folder watcher** — ~~watch `<vault>/raw/_inbox/`~~ | Shipped: `autoIngest.ts` polls `<vault>/_inbox/` on an interval while the app is open, and `automation/autoingest.py` drains it headless. **The path in this row was wrong** — the inbox is `_inbox/` beside `raw/`, never inside it. What is genuinely left: the in-app pass sees only `.md` (`list_files` is markdown-only) and **deletes** the consumed source where the daemon archives it — fix that divergence before any bulk run. | **S** (the remaining slice) | `app/src/lib/autoIngest.ts`, `automation/autoingest.py` |
| **P1 — phases 1+2 DONE** | **Wikification pipeline v2** — chunk → retrieve candidate pages → explicit ADD/UPDATE/MERGE/NOOP plan → cite (select-then-generate) | The single-prompt "Claude, ingest this" approach does not scale to long transcripts or dedup across hundreds of sessions; this is the quality core. **Phase 1 (retrieval grounding)**: `pipeline::rank_candidates` + `wikify_candidates` chunk the source (`embeddings::chunk_page`), embed with the index's own model, retrieve top-k existing pages (`VectorStore::search`) and inject them into the ingest prompt — the dedup-across-sessions core, on the existing local-embedding infra (no new dep). **Phase 2 (plan preview)**: one read-only planning call (`ingestPlan.ts`) turns the source + candidates into explicit per-topic ADD/UPDATE/MERGE/NOOP decisions, shown as telemetry and fed to the agent; prompt-and-parse JSON (no schema-constrained output exists), best-effort. Frontend-orchestrated (matches ingest's grain) — the agent still writes; a full Rust-applies-writes orchestrator was **deliberately not built** (LLM authoring content is a quality risk). **Open:** end-to-end plan/extraction quality needs tuning on a real vault + model — cannot be validated headlessly. | **L** | `src-tauri/src/pipeline.rs`, `commands::wikify_candidates`, `lib/ingestPlan.ts`, `ingestStore.ts`; embeddings via the bundled local model |
| ~~**P1**~~ **DONE** | **Bulk-import UX** — file-level progress bar + running imported/skipped/failed tally, a collapsible failures list, and "retry failed only" that re-reads only the failed files (importPaths, not a whole re-sweep) | Shipped: importStore consumes an `import-progress` event stream (throttled); ConversationImport shows the bar, tally and failures on the Ingest card | **—** | `commands::run_import`/`import_paths`, `stores/importStore.ts`, `ConversationImport.tsx` |
...
| **P2** | **Scheduled / auto ingest of session dirs** — periodic sweep of `~/.claude/projects` + `~/.codex/sessions` for new `*.jsonl` since last run | Coding sessions are produced continuously; the wiki should self-update without a manual drop. **Note:** scheduled ingest itself already exists (`settings.auto_ingest_interval_min`, `schedules.rs`, `PageSchedules.tsx`) — what is missing is sweeping the *session directories*, which is blocked on the parsers since nothing reads `.jsonl` today. | **M** | `schedules.rs` + the parsers |
...
| ~~**P2**~~ **DONE (file-level)** | **Incremental session tailing** — ~~only ingest new lines since last byte offset~~ | Shipped a **file-level skip** instead: the ledger stamps each session's `(mtime, len)`, and a re-sweep skips an unchanged file *without reading it*, counting its conversations as already imported. Byte-offset tailing doesn't fit the model — a Claude Code session is ONE conversation that needs the whole file to rebuild its turns — so a whole-file skip is the right optimization; re-sweeping thousands of sessions is now near-instant. | **S** | `importers/ledger.rs` (files map) + `commands::run_import` |
| ~~**P2**~~ **DONE** | **Import provenance surfacing** — the Provenance page now lists each page's cited sources, resolving every `[^src-<slug>]` to its `raw/<slug>.md` frontmatter: an imported source shows its vendor (ChatGPT / Claude.ai / Claude Code / Codex), conversation id and date; a hand-authored one its title; a dangling citation is flagged missing | Shipped read-side (no ingest change): `scan_provenance` collects per-page source slugs + a raw index; `SourceRef` on `ProvenanceRow`; expandable source list in `PageProvenance.tsx` | **—** | `provenance.rs` + `PageProvenance.tsx` |
...
Two-level key, stored in `<vault>/.myco/ledger.json` (gitignored):
...
- **Data flow:** two layers. (a) In-app: a Rust background task on an interval (or a Tauri `watch` on the two session dirs) finds `*.jsonl` newer than `last_sweep` and feeds them to the pipeline. (b) Headless: a `launchd` plist (macOS) / systemd `.timer` (Linux — preferred over cron: `Persistent=` catches up after downtime, journald logging) invoking `myco_import.py --since-last-sweep --source claude-code,codex`.
...
- Append-only, **one JSON object per line**. Encoded dir = absolute cwd with `/` and `.` → `-` (e.g. `-Users-yoo-project-myco`). Line `type` ∈ `{user, assistant, system, summary, file-history-snapshot, attachment, mode, last-prompt, ai-title, queue-operation}`.
