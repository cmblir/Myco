...
describe("applyMyceliumLayout", () => {
  // A thread: hub → a → a1 → a2 … plus siblings that should peel off sideways.
  const build = (): VaultGraph => {
    const g = makeGraph([
      { id: "hub.md", community: 0, deg: 3 },
      { id: "a.md", community: 0, deg: 2 },
      { id: "a1.md", community: 0, deg: 2 },
      { id: "a2.md", community: 0, deg: 1 },
      { id: "b.md", community: 1, deg: 1 },
      { id: "c.md", community: 1, deg: 1 },
      { id: "lonely.md", community: -1, deg: 0 },
    ]);
    g.addEdge("hub.md", "a.md");
    g.addEdge("hub.md", "b.md");
    g.addEdge("hub.md", "c.md");
    g.addEdge("a.md", "a1.md");
    g.addEdge("a1.md", "a2.md");
    return g;
  };

  it("gives every node a finite position and roots the busiest hub at the centre", () => {
    const g = build();
    applyMyceliumLayout(g, { targetRadius: 500 });
    const hub = pos(g, "hub.md");
    expect(Math.hypot(hub.x, hub.y, hub.z)).toBeLessThan(120);
    g.forEachNode((id) => {
      const p = pos(g, id);
      expect(Number.isFinite(p.x) && Number.isFinite(p.y) && Number.isFinite(p.z)).toBe(true);
    });
  ...
  it("keeps a lineage going in roughly one direction — the point of apical dominance", () => {
    // hub → a → a1 → a2 is the heaviest chain. Each step must continue the
    // previous heading rather than radiate, or this is just another tree layout.
    const g = build();
    applyMyceliumLayout(g, { targetRadius: 500 });
    const v = (from: string, to: string): number[] => {
      const p = pos(g, from);
      const q = pos(g, to);
    ...
    const s1 = v("hub.md", "a.md");
    const s2 = v("a.md", "a1.md");
    const s3 = v("a1.md", "a2.md");
    const dot = (x: number[], y: number[]): number => x[0] * y[0] + x[1] * y[1] + x[2] * y[2];
    // Consecutive segments stay within ~60° — a thread, not a starburst.
    expect(dot(s1, s2)).toBeGreaterThan(0.5);
    expect(dot(s2, s3)).toBeGreaterThan(0.5);
  });

  it("sends a lateral branch off the thread, not along it", () => {
    const g = build();
    applyMyceliumLayout(g, { targetRadius: 500 });
    const v = (from: string, to: string): number[] => {
      const p = pos(g, from);
    ...
    const apical = v("hub.md", "a.md");
    const lateral = v("hub.md", "b.md");
    const dot =
      apical[0] * lateral[0] + apical[1] * lateral[1] + apical[2] * lateral[2];
    // A lateral leaves at a real angle — if this approached 1 the branches
    // would be stacked on the thread and invisible.
    expect(dot).toBeLessThan(0.85);
  });

  it("starts a disconnected component as its own colony, away from the main root", () => {
    const g = build();
    applyMyceliumLayout(g, { targetRadius: 500 });
    const p = pos(g, "lonely.md");
    expect(Math.hypot(p.x, p.y, p.z)).toBeGreaterThan(60);
  });

  it("is deterministic — the same vault lays out identically twice", () => {
    const a = build();
    const b = build();
    applyMyceliumLayout(a, { targetRadius: 500 });
    applyMyceliumLayout(b, { targetRadius: 500 });
    a.forEachNode((id) => {
      expect(pos(b, id)).toEqual(pos(a, id));
    });
  });

  it("keeps nodes apart instead of piling them up", () => {
    // 200 leaves on one hub: without the avoidance pass they land on top of
    // each other and the mat reads as a single lump.
    const nodes = [{ id: "root", community: 0, deg: 200 }];
    for (let i = 0; i < 200; i++) nodes.push({ id: `n${i}`, community: i % 5, deg: 1 });
    const g = makeGraph(nodes);
    for (let i = 0; i < 200; i++) g.addEdge("root", `n${i}`);
    applyMyceliumLayout(g, { targetRadius: 600 });
    let tooClose = 0;
    for (let i = 0; i < 200; i++) {
      for (let j = i + 1; j < 200; j++) {
        const p = pos(g, `n${i}`);
        const q = pos(g, `n${j}`);
        if (Math.hypot(p.x - q.x, p.y - q.y, p.z - q.z) < 1) tooClose++;
      }
    }
    expect(tooClose).toBe(0);
  });

  it("spreads as a mat — wider than it is tall", () => {
    // FLATTEN exists so mycelium never reads as walrus's ball at a glance.
    const nodes = [{ id: "root", community: 0, deg: 60 }];
    for (let b = 0; b < 6; b++) {
      nodes.push({ id: `b${b}`, community: b, deg: 11 });
      for (let i = 0; i < 10; i++) nodes.push({ id: `b${b}_${i}`, community: b, deg: 1 });
    }
    const g = makeGraph(nodes);
    for (let b = 0; b < 6; b++) {
      g.addEdge("root", `b${b}`);
      for (let i = 0; i < 10; i++) g.addEdge(`b${b}`, `b${b}_${i}`);
    }
    applyMyceliumLayout(g, { targetRadius: 600 });
    let maxXZ = 0;
    let maxY = 0;
    g.forEachNode((id) => {
      const p = pos(g, id);
      maxXZ = Math.max(maxXZ, Math.hypot(p.x, p.z));
      maxY = Math.max(maxY, Math.abs(p.y));
    });
    expect(maxXZ).toBeGreaterThan(maxY * 1.3);
  });
});
