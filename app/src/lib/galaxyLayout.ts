...
import { ORBIT_BASE, ORBIT_GROW } from "./layoutConfig";
...
// The worker's per-cluster orbit ring radius (leaves orbit their hub at this
// distance, × the member's own 0.4..1.0 jitter). Defined HERE from the shared
// layoutConfig constants so the packing footprint below and the force that
// fills it can never drift apart again (they did: a stale hand-mirrored copy
// of these constants said 0.35/0.07 while the worker ran 0.32/0.06).
export function clusterOrbitRadius(
  count: number,
  linkDistance: number,
  sizeBoost = 1,
): number {
  return linkDistance * (ORBIT_BASE + ORBIT_GROW * Math.sqrt(Math.max(1, count))) * sizeBoost;
}
